...
class ResultController(sceneManager: SceneManager) extends Initializable {
      ...
      def setResults(mode:String, finalScore:Int): Unit = {
      ...
      // PURPOSE: Set up the visual effects for the buttons
      private def setupButtonEffects(): Unit = {
            val buttons = List(
                  Option(tryAgainButton),
                  Option(tryOtherModeButton),
                  Option(mainMenuButton)
            ).flatten
            
            buttons.foreach(button => {
                  button.setOnMouseEntered(_ => {
                  FxEffects.applyPulseEffect(button)
                  })
                  
                  button.setOnMouseExited(_ => {
                  FxEffects.removeEffect(button)
                  })
            })
      }
}